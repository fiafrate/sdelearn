...
class Qmle(SdeLearner):
    ...
    def hessian(self, param, batch_id=None, **kwargs):
        ...
        hess_ab = np.sum(np.einsum('nd, npde, neq -> npq', (self.DX[self.batch_id] - dn * self.bs)[:, 0, :], GB2a, Jbs), axis=0)

        hess_i = -2 * np.block([[hess_alpha, hess_ab], [hess_ab.transpose(), hess_beta]]) * 1 / len(self.batch_id)
        return hess_i
